...
#define freeurl @"http://1000phone.net:8088/app/iAppFree/api/reduce.php"



@interface ReduceViewController ()<UITableViewDataSource,UITableViewDelegate,UISearchResultsUpdating,UISearchBarDelegate>
{
    UITableView *ReduceTableView;
...
- (void)requestMessage:(NSString *)url parameters:(NSDictionary *)parameters
{
    ...
    [manager GET:url parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        [ReduceTableView reloadData];
        NSLog(@"%@",dataArray);
        [ReduceTableView.mj_footer endRefreshing];
        [ReduceTableView.mj_header endRefreshing];
        hud.hidden = YES;
...
- (void)setUI
{
    ...
    //创建TableView
    ReduceTableView = [[UITableView alloc] initWithFrame:self.view.frame style:UITableViewStylePlain];
    ReduceTableView.delegate = self;
    ReduceTableView.dataSource = self;
    [self.view addSubview:ReduceTableView];
    
    
    //上拉刷新
    ReduceTableView.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
        [self requestMessage:freeurl parameters:nil];
    }];
    
    //下拉刷新
    ReduceTableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        page = 0;
        [self requestMessage:freeurl parameters:nil];
    }];
    ...
    ReduceTableView.tableHeaderView = serch.searchBar;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.timelabel.text = [NSString stringWithFormat:@"现价:￥%@.00",dataArray[indexPath.row][@"currentPrice"]];
