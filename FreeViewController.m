...
#define freeurl @"http://1000phone.net:8088/app/iAppFree/api/free.php"

@interface FreeViewController ()<UITableViewDataSource,UITableViewDelegate,UISearchResultsUpdating,UISearchBarDelegate>
{
    UITableView *FreeTableView;
...
//    NSString *urlstring = @"http://1000phone.net:8088/app/iAppFree/api/free.php";
    ...
    [manager GET:url parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        NSLog(@"%@",dataArray);
        [FreeTableView.mj_footer endRefreshing];
        [FreeTableView.mj_header endRefreshing];
        hud.hidden = YES;
...
- (void)setUI
{
    ...
    //创建TableView
    FreeTableView = [[UITableView alloc] initWithFrame:self.view.frame style:UITableViewStylePlain];
    FreeTableView.delegate = self;
    FreeTableView.dataSource = self;
    [self.view addSubview:FreeTableView];
    
    //上拉刷新
    FreeTableView.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
        [self requestMessage:freeurl parameters:nil];
    }];
    
    //下拉刷新
    FreeTableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        page = 0;
        [self requestMessage:freeurl parameters:nil];
    }];
    ...
    FreeTableView.tableHeaderView = serch.searchBar;
}
