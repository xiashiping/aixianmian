...
@interface SubjectViewController ()<UITableViewDataSource,UITableViewDelegate,UISearchResultsUpdating,UISearchBarDelegate>
{
...

{
    ...
    [manager GET:url parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        NSLog(@"%@",dataArray);
        [SubjectTableView.mj_footer endRefreshing];
        [SubjectTableView.mj_header endRefreshing];
        hud.hidden = YES;
...
- (void)setUI
{
    ...
    //创建TableView
    SubjectTableView = [[UITableView alloc] initWithFrame:self.view.frame style:UITableViewStylePlain];
    SubjectTableView.delegate = self;
    SubjectTableView.dataSource = self;
    [self.view addSubview:SubjectTableView];
    
    //上拉刷新
    SubjectTableView.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
        [self requestMessage:freeurl parameters:nil];
    }];
    ...
    SubjectTableView.tableHeaderView = serch.searchBar;
}
